...
namespace MesibaViewer.ViewModel
{
    public class PartyImageViewModel
    {
        ...
        public void SetFirstPic(object sender, RoutedEventArgs e)
        {
            ((FlipView)sender).SelectedIndex = 1;
            flipView = (FlipView) sender;
        }
        public void LoadLocationBar(object sender, RoutedEventArgs e)
        {
            locationBar = (StackPanel) sender;
            foreach(Image pic in previewPics)
            {
                locationBar.Children.Add(pic);
            }
            locationBar.UpdateLayout();
